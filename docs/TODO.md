...
- Dedicated-server multiplayer support hardening: test Servux capture/paste, including raw bulk entity capture for server-visible entities the client is not tracking; test command-mode lossy commits/applies, no-Servux permission failures, chunk unknown handling, and overlay/verifier refresh after remote operations.
...
- Scheduled ticks are intentionally not stored. Discard/checkout/clear no longer freeze or suppress scheduled work; they do one direct rewrite and rely on Litematica paste-style update suppression around block writes.
...
- Pull restore can still trigger redstone contraptions mid-restore because it has not moved to the discard/selected-checkout/branch-checkout scan/rewrite/client-sync restore path yet.
...
- Avoid unnecessary neighbor updates/redstone ticks mid-restore when the intended operation is restoring a static committed schematic state. Use the discard/selected-checkout/branch-checkout scan/rewrite/client-sync path as the current reference for future pull restore work.
...
- Refresh the normal overlay verifier after restore. Restore operations do not run a separate post-operation verification scan or show a mismatch popup; users can run Scan Changes explicitly when they want an authoritative review.
...
- Checkout preflight blocks on uncommitted Git/world changes with an Unsaved Changes popup; clean checkouts still do not yet show rich affected-region previews.
...
- Singleplayer restore uses integrated-server direct world writes through the scan/rewrite/client-sync restore engine.
