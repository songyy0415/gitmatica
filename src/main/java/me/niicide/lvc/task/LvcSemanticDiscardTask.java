...
public final class LvcSemanticDiscardTask extends LvcChunkedTaskBase<LvcSemanticDiscardTask.Result>
{
    ...
    private int frozenTicksToRestore;
    ...
    private void prepareWorldMutation() throws Exception
    {
        this.markOperationWillDiscard();
        this.clearScheduledActivityIfNeeded();
    }

    private void acquireTickFreezeIfNeeded(ServerLevel serverWorld)
    {
        if (this.journalOperation != LvcOperationJournal.Operation.DISCARD)
        {
            return;
        }

        ServerTickRateManager manager = serverWorld.getServer().tickRateManager();
        this.tickRateManager = manager;
        this.wasTickFrozen = manager.isFrozen();
        this.frozenTicksToRestore = manager.frozenTicksToRun();
        this.tickFreezeAcquired = true;
        ...
        LvcDiagnostics.debug(this.handle(),
                ...
                this.wasTickFrozen, this.frozenTicksToRestore);
    }

    private void clearScheduledActivityIfNeeded() throws IOException
    {
        if (!this.tickFreezeAcquired || this.scheduledActivityCleared)
        {
            return;
        }

        ServerLevel serverWorld = (ServerLevel) this.world;
        LvcIntPosition origin = Objects.requireNonNull(this.origin, "origin");
        int blockTicksBefore = serverWorld.getBlockTicks().count();
        int fluidTicksBefore = serverWorld.getFluidTicks().count();
        int regionCount = 0;

        try
        {
            for (LvcManifest.Region region : this.requireSite().regions())
            {
                BoundingBox bounds = trackedWorldBounds(region, origin);
                serverWorld.getBlockTicks().clearArea(bounds);
                serverWorld.getFluidTicks().clearArea(bounds);
                serverWorld.clearBlockEvents(bounds);
                regionCount++;
            }
        }
        ...
        LvcDiagnostics.debug(this.handle(),
                ...
                regionCount,
                Math.max(0, blockTicksBefore - serverWorld.getBlockTicks().count()),
                Math.max(0, fluidTicksBefore - serverWorld.getFluidTicks().count()));
    }

    private void releaseTickFreeze()
    {
        ServerTickRateManager manager = this.tickRateManager;

        if (!this.tickFreezeAcquired || manager == null)
        {
            return;
        }

        this.tickFreezeAcquired = false;
        MinecraftServer server = ((ServerLevel) this.world).getServer();
        Runnable restore = () ->
        {
            if (manager.isSteppingForward())
            {
            ...
            manager.setFrozen(this.wasTickFrozen);

            if (this.wasTickFrozen && this.frozenTicksToRestore > 0)
            {
                manager.stepGameIfPaused(this.frozenTicksToRestore);
            }

            LvcDiagnostics.debug(this.handle(),
                    "semantic discard restored global tick state frozen={} stepTicks={}",
                    this.wasTickFrozen, this.wasTickFrozen ? this.frozenTicksToRestore : 0);
        };
