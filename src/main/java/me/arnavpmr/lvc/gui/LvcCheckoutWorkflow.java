...
import me.arnavpmr.lvc.task.LvcSemanticScanTask;
...
final class LvcCheckoutWorkflow
{
    ...
    static void promptCheckoutCommit(GuiLvcProjectController controller, LvcCommitInfo commit)
    {
    ...
    static void checkoutCommit(GuiLvcProjectController controller, LvcCommitInfo commit)
    {
    ...
    static void promptCheckoutBranch(GuiLvcProjectController controller, String branchName)
    {
        ...
        try
        {
            String targetCommitId = LvcGitBranchOps.localBranchTipCommitId(controller.gui.repositoryDirectory, trimmedBranchName);
            ...
            if (LvcGitBranchOps.headMatchesCommit(controller.gui.repositoryDirectory, targetCommitId))
            {
        ...
        {
            controller.gui.syncBranchDropdownSelection();
            LvcGuiMessages.showTaskError(Operation.CHECKOUT_BRANCH, "gitmatica.error.lvc_project.checkout_failed", e);
    ...
    private static void checkoutBranchWithoutRestore(GuiLvcProjectController controller, String branchName)
    {
        try
        {
            LvcGitBranchOps.checkoutBranchToWorkingTree(controller.gui.repositoryDirectory, branchName);

            if (LvcTrackingOverlayService.isCurrentTrackingOverlayLoaded(controller.gui.repositoryDirectory))
            {
            ...
            LvcGuiMessages.show(MessageType.SUCCESS, "gitmatica.message.lvc_project.switched_branch", branchName);
        ...
        LvcSemanticCheckoutTask.Preflight task = new LvcSemanticCheckoutTask.Preflight(
                ...
                LvcTaskCallbacks.of(
                        ...
                        e ->
                        {
                            ...
                            LvcGuiMessages.showTaskError(target.operation(), "gitmatica.error.lvc_project.checkout_failed", e);
                        },
                        () ->
                        {
                            target.syncBranchDropdownIfNeeded(controller);
                            LvcGuiMessages.show(MessageType.INFO, "gitmatica.message.lvc_project.task_aborted", target.taskName());
                        }
                )
        );
        ...
        try
        {
            if (scan.unknownChunks() > 0)
            {
                ...
                LvcGuiMessages.showUnloadedTrackedChunks(target.operation(),
                        "gitmatica.error.lvc_project.checkout_failed", scan.unknownChunks());
                return;
            }

            boolean gitDirty = LvcGitBranchOps.hasUncommittedChanges(controller.gui.repositoryDirectory);
    ...
    {
        if (!LvcConfigs.Generic.SHOW_CHECKOUT_WARNING.getBooleanValue())
        {
        ...
        GuiBase.openGui(new GuiLvcWarningConfirmDialog(
                ...
                "gitmatica.gui.title.lvc_project.confirm_checkout",
                "gitmatica.gui.message.lvc_project.confirm_checkout",
                LvcConfigs.Generic.SHOW_CHECKOUT_WARNING,
                target.taskName(),
                () -> schedulePreparedCheckout(controller, handle, prepared, target),
                () ->
                {
        ...
        try
        {
            ...
            LvcSemanticCheckoutTask.Apply task = new LvcSemanticCheckoutTask.Apply(
                    ...
                    LvcTaskCallbacks.of(
                            ...
                            {
                                target.syncBranchDropdownIfNeeded(controller);
                                LvcGuiMessages.showTaskError(target.operation(), "gitmatica.error.lvc_project.checkout_failed", e, true);
                            },
                            () ->
                            {
                                target.syncBranchDropdownIfNeeded(controller);
    ...
    {
        private static CheckoutTarget commit(LvcCommitInfo commit)
        {
        ...
        private boolean reattachIfNeeded(GuiLvcProjectController controller)
        {
            ...
            try
            {
                LvcGitBranchOps.reattachHeadToBranchIfAtTip(controller.gui.repositoryDirectory,
                        controller.gui.checkoutBranchName);
                return true;
            }
            catch (Exception e)
            {
                LvcGuiMessages.showTaskError(this.operation(), "gitmatica.error.lvc_project.checkout_failed", e);
                return false;
            }
        }
        ...
        private void showSuccess(int regionCount)
        {
            if (this.branchSwitch)
            {
                LvcGuiMessages.show(MessageType.SUCCESS, "gitmatica.message.lvc_project.checked_out_branch",
                        this.displayName, LvcOperationCoordinator.regionCountText(regionCount));
                return;
            }

            LvcGuiMessages.show(MessageType.SUCCESS, "gitmatica.message.lvc_project.checked_out",
                    this.displayName, LvcOperationCoordinator.regionCountText(regionCount));
        }
