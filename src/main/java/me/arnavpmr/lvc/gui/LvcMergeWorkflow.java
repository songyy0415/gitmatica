...
import me.arnavpmr.lvc.git.LvcBranchMergeOps;
import me.arnavpmr.lvc.git.LvcGitBranchOps;
import me.arnavpmr.lvc.git.LvcBranchMergeResult;
import me.arnavpmr.lvc.git.LvcBranchMergeConflictResolution;
import me.arnavpmr.lvc.git.LvcBranchMergeStatus;
...
final class LvcMergeWorkflow
{
    ...
    static void mergeBranch(GuiLvcProjectController controller, String sourceBranch)
    {
            ...
            LvcSemanticScanTask task = new LvcSemanticScanTask(
                    ...
                    LvcTaskCallbacks.of(
                            ...
                            e -> LvcGuiMessages.showTaskError(Operation.MERGE_BRANCH,
                                    "gitmatica.error.lvc_project.merge_branch_failed", e),
        ...
        catch (Exception e)
        {
            ...
            LvcGuiMessages.showTaskError(Operation.MERGE_BRANCH, "gitmatica.error.lvc_project.merge_branch_failed", e);
        }
    }
        ...
        try
        {
            if (scan.unknownChunks() > 0)
            {
                ...
                LvcGuiMessages.showUnloadedTrackedChunks(Operation.MERGE_BRANCH,
                        "gitmatica.error.lvc_project.merge_branch_failed", scan.unknownChunks());
                return;
            }
            ...
            if (result.status() == LvcBranchMergeStatus.UP_TO_DATE)
            {
                LvcTaskRegistry.release(handle);
                ...
                LvcGuiMessages.show(MessageType.INFO, "gitmatica.message.lvc_project.merge_branch_up_to_date",
                                                ...
                                                LvcMergeConflictException conflict)
    {
        ...
        GuiBase.openGui(new GuiLvcMergeConflictDialog(
                ...
                {
                    LvcTaskRegistry.release(handle);
                    LvcGuiMessages.show(MessageType.ERROR, "gitmatica.error.lvc_project.merge_branch_conflicts");
                }
        ));
    }
    ...
    private static void resolveMergeConflict(GuiLvcProjectController controller, LvcOperationHandle handle,
                                             Level restoreWorld, String sourceBranch, Player player,
                                             LvcBranchMergeConflictResolution resolution)
    {
        try
        {
            if (LvcGitBranchOps.hasUncommittedChanges(controller.gui.repositoryDirectory))
            {
                LvcTaskRegistry.release(handle);
                LvcOperationCoordinator.showUnsavedChangesNotice(controller, "LVC Merge Branch");
                return;
    ...
    private static void scheduleMergeRestore(GuiLvcProjectController controller, LvcOperationHandle handle,
                                             Level restoreWorld, LvcBranchMergeResult merge) throws IOException
    {
                ...
                LvcTaskCallbacks.of(
                        result ->
                        {
                            ...
                            LvcGuiMessages.show(MessageType.SUCCESS,
                                    merge.status() == LvcBranchMergeStatus.FAST_FORWARD ?
                                            "gitmatica.message.lvc_project.merge_branch_fast_forward" :
                                            "gitmatica.message.lvc_project.merge_branch_merged",
                                    merge.sourceBranch(), merge.targetBranch(),
                                    LvcOperationCoordinator.regionCountText(result.restoredRegionCount()));
                        },
                        e -> LvcGuiMessages.showTaskError(Operation.MERGE_BRANCH,
                                "gitmatica.error.lvc_project.merge_branch_failed", e, true),
                        () -> LvcGuiMessages.show(MessageType.INFO,
                                "gitmatica.message.lvc_project.task_aborted", "LVC Merge Branch")
                ),
                "LVC Merge Branch",
    ...
    private static void scheduleRemoteMergeRestore(GuiLvcProjectController controller, LvcOperationHandle handle,
                                                   Level restoreWorld, LvcBranchMergeResult merge)
    {
        try
        {
            controller.removeTrackingOverlay();
