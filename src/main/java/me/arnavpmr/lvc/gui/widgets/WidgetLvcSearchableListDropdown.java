...
public abstract class WidgetLvcSearchableListDropdown<T> extends WidgetBase
{
    ...
    private void renderClosedValue(GuiContext ctx)
    {
        ...
        {
            String displayedLabel = LvcGuiText.ellipsizeToWidth(label, textWidth, this::getStringWidth);
    ...
    private void renderRows(GuiContext ctx, int mouseX, int mouseY)
    {
        ...
        for (int index = this.scrollOffset; index < endIndex; index++)
        {
            ...
            this.drawString(ctx, textX, y + 5, 0xFFFFFFFF,
                    LvcGuiText.ellipsizeToWidth(this.itemLabel(item), textWidth, this::getStringWidth));
            y += ROW_HEIGHT;
        }
    }
    ...
    private void renderHoveredItemTooltip(GuiContext ctx, int mouseX, int mouseY)
    {
        ...
        if (this.getStringWidth(label) > textWidth)
        {
            ...
            RenderUtils.drawHoverText(ctx, mouseX, mouseY,
                    LvcGuiText.wrapTextToWidth(label, maxWidth, this::getStringWidth));
        }
    }
