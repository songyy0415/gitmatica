...
public class WidgetLvcProjectSubRegion extends WidgetListEntryBase<LvcManifest.Region>
{
    ...
    public void render(GuiContext ctx, int mouseX, int mouseY, boolean selected)
    {
        ...
        if (this.entry != null)
        {
            String name = LvcGuiText.ellipsizeToWidth(
                    this.entry.name(),
                    Math.max(24, this.buttonsStartX - this.x - 8),
                    this::getStringWidth
            );
    ...
    public void postRenderHovered(GuiContext ctx, int mouseX, int mouseY, boolean selected)
    {
        if (this.entry != null && GuiBase.isMouseOver(mouseX, mouseY, this.x, this.y, this.buttonsStartX - this.x - 10, this.height))
        {
            ...
            text.add(StringUtils.translate("gitmatica.gui.label.lvc_project_editor.region_min") + ": " + this.vectorText(this.entry.min()));
            text.add(StringUtils.translate("gitmatica.gui.label.lvc_project_editor.region_size") + ": " + this.vectorText(this.entry.size()));
