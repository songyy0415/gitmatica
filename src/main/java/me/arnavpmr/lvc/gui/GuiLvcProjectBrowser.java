...
import me.arnavpmr.lvc.semantic.LvcSemanticProjectOperations;
import me.arnavpmr.lvc.semantic.LvcEmptyProjectResult;
...
import me.arnavpmr.lvc.gui.widgets.WidgetLvcProjectRepositoryBrowser;
...
public class GuiLvcProjectBrowser extends GuiListBase<DirectoryEntry, WidgetDirectoryEntry, WidgetLvcProjectRepositoryBrowser>
        implements ISelectionListener<DirectoryEntry>
{
    ...
    public GuiLvcProjectBrowser()
    {
        ...
        this.title = StringUtils.translate("gitmatica.gui.title.lvc_project_browser", LvcReference.MOD_VERSION);
    }
    ...
    {
        return this.getPanelLeftX() + WidgetLvcProjectRepositoryBrowser.getBrowserPanelWidthForTotalWidth(this.getBrowserWidth());
    }
    ...
    @Override
    protected WidgetLvcProjectRepositoryBrowser createListWidget(int listX, int listY)
    {
        return new WidgetLvcProjectRepositoryBrowser(listX, listY, 100, 100, this.getSelectionListener());
    }
    ...
    private void openSelectedProject()
    {
        ...
        if (entry == null)
        {
            this.addMessage(MessageType.INFO, "gitmatica.message.lvc_project_manager.no_project_selected");
            return;
        }
    ...
    private void confirmDeleteSelectedProject()
    {
        ...
        GuiBase.openGui(new GuiLvcConfirmAction(
                420,
                "gitmatica.gui.title.lvc_project_manager.confirm_delete_project",
                deleter,
                this,
                "gitmatica.gui.message.lvc_project_manager.confirm_delete_project",
                entry.name()
        ));
    }
    ...
    private record ProjectDeleter(Path repositoryDirectory, String projectName, GuiLvcProjectBrowser gui) implements IConfirmationListener
    {
        ...
        public boolean onActionConfirmed()
        {
            try
            {
                LvcProjectCatalog.deleteProject(Minecraft.getInstance().gameDirectory.toPath(), this.repositoryDirectory);
                WidgetLvcProjectRepositoryBrowser listWidget = this.gui.getListWidget();
                ...
                LvcGuiMessages.show(MessageType.SUCCESS, "gitmatica.message.lvc_project_manager.project_deleted", this.projectName);
                return true;
            }
            catch (IOException | RuntimeException e)
            {
                LvcGuiMessages.showTaskError(Operation.PROJECT_BROWSER, "gitmatica.error.lvc_project_manager.delete_failed", e);
                return false;
            }
        }
    ...
    private record ProjectCreator(GuiLvcProjectBrowser gui)
    {
        ...
        private void createProject(String projectName)
        {
            try
            {
                LvcEmptyProjectResult result = LvcSemanticProjectOperations.createEmptyProject(
                        Minecraft.getInstance().gameDirectory.toPath(),
                        projectName,
                ...
                this.gui.reCreateGuiElements();
                LvcGuiMessages.show(MessageType.SUCCESS, "gitmatica.message.lvc_project_manager.project_created", result.projectName());
            }
            catch (FileAlreadyExistsException e)
            {
                LvcDiagnostics.debug("GuiLvcProjectBrowser: create project failed after popup input='{}' error='exists'",
                        projectName);
                LvcGuiMessages.show(MessageType.ERROR, "gitmatica.error.lvc_project_manager.project_name_used");
            }
            catch (Exception e)
            {
                LvcDiagnostics.debug("GuiLvcProjectBrowser: create project failed after popup input='{}' error='{}'",
                        projectName, e.getMessage());
                LvcGuiMessages.showTaskError(Operation.PROJECT_BROWSER, "gitmatica.error.lvc_project.create_failed", e);
            }
        }
    ...
    private record ButtonListener(Type type, GuiLvcProjectBrowser gui) implements IButtonActionListener
    {
        ...
        public void actionPerformedWithButton(ButtonBase button, int mouseButton)
        {
            switch (this.type)
            {
                case CREATE_PROJECT -> this.gui.showNotImplemented("gitmatica.message.lvc_project_manager.create_project_not_implemented");
                case CLONE_PROJECT -> this.gui.showNotImplemented("gitmatica.message.lvc_project_manager.clone_project_not_implemented");
        ...
        private enum Type
        {
            CREATE_PROJECT("gitmatica.gui.button.lvc_project.create_short"),
            CLONE_PROJECT("gitmatica.gui.button.lvc_project.clone"),
            OPEN_PROJECT("gitmatica.gui.button.lvc_project.open_project"),
            DELETE_PROJECT("gitmatica.gui.button.lvc_project.delete"),
