...
final class GuiLvcProjectCommandListeners
{
    ...
    record RemoteUrlSetter(GuiLvcProjectController controller, boolean pushAfterSave) implements IStringConsumerFeedback
    {
        ...
        public boolean setString(String remoteUrl)
        {
            try
            {
                LvcGitRemoteOps.setRemote(this.controller.gui.repositoryDirectory, remoteUrl);
                this.controller.refreshRepositoryState();

                if (this.pushAfterSave)
                {
                    LvcGitRemoteOps.push(this.controller.gui.repositoryDirectory);
                    LvcGuiMessages.show(MessageType.SUCCESS, "gitmatica.message.lvc_project.pushed");
                }
                else
                {
                    this.controller.gui.initGui();
                    LvcGuiMessages.show(MessageType.SUCCESS, "gitmatica.message.lvc_project.remote_updated", remoteUrl.trim());
                }
            ...
            {
                LvcGuiMessages.show(MessageType.ERROR, this.pushAfterSave ? "gitmatica.error.lvc_project.push_failed" : "gitmatica.error.lvc_project.remote_failed", LvcGitRemoteOps.describeRemoteFailure(e));
                return false;
            }
        }
    }
    ...
    record CommitMessageSetter(GuiLvcProjectController controller) implements IStringDualConsumerFeedback
    {
        ...
        public boolean setStrings(String title, String description)
        {
            ...
            {
                LvcGuiMessages.show(MessageType.ERROR, "gitmatica.error.lvc_project.commit_failed", "Commit message must not be blank");
                return false;
            }
    ...
    record ResetAndCheckoutCommitConfirmListener(GuiLvcProjectController controller, LvcCommitInfo commit) implements IConfirmationListener
    {
        @Override
        public boolean onActionConfirmed()
        {
