...
import me.arnavpmr.lvc.util.LvcGuiTextFields;
...
class GuiLvcCommitMessageDialog extends GuiTextInputStackedMultiLine
{
    ...
    protected int createButton(int x, int y, ButtonType type)
    {
        String label = type == ButtonType.OK ?
                StringUtils.translate("gitmatica.gui.button.lvc_project.save") :
                type.getDisplayName();
    ...
    public void drawContents(GuiContext ctx, int mouseX, int mouseY, float partialTicks)
    {
        ...
        this.drawStringWithShadow(ctx, StringUtils.translate("gitmatica.gui.label.lvc_project.commit_description"),
                this.dialogLeft + 10, this.textField2.getY() - DESCRIPTION_LABEL_FIELD_OFFSET, COLOR_WHITE);

        if (this.textField1.getValue().isEmpty())
        {
            ctx.drawString(ctx.fontRenderer(), StringUtils.translate("gitmatica.gui.label.lvc_project.commit_title_placeholder"),
                    this.textField1.getX() + 4, this.textField1.getY() + 6, 0xFF777777, false);
        }

        if (this.textField2.getValue().isEmpty())
        {
            ctx.drawString(ctx.fontRenderer(), StringUtils.translate("gitmatica.gui.label.lvc_project.commit_description_optional"),
    ...
    protected boolean applyValues(String title, String description)
    {
        ...
        {
            this.errorMessage = StringUtils.translate("gitmatica.error.lvc_project.commit_title_required");
            this.showErrorSpace();
            this.textField1.setFocused(true);
            this.textField2.setFocused(false);
            this.selectedBox = 1;
