...
public final class LvcSemanticDiscardTask extends LvcChunkedTaskBase<LvcSemanticDiscardTask.Result>
{
    ...
    @Nullable private LvcTickFreezeScope tickFreezeScope;
    ...
    public void init()
    {
        try
        {
            ...
            this.commit = LvcGitTreeReader.resolveCommit(repository, this.revWalk, commitId);
            ...
            {
                this.tickFreezeScope = LvcTickFreezeScope.acquire(serverWorld, this.handle());
            }
    ...
    protected void updateProgressHud()
    {
        ...
        LvcSemanticRestoreEngine engine = this.restoreEngine;
        this.infoHudLines.add(StringUtils.translate("gitmatica.gui.label.lvc_project.task_phase", engine == null ? "restore" : engine.phaseLabel()));
    ...
    public void stop()
    {
        try
        {
            if (this.tickFreezeScope != null)
            {
                this.tickFreezeScope.close();
                this.tickFreezeScope = null;
            }
        }
        finally
        {
    ...
    {
        this.markOperationWillDiscard();
        LvcTickFreezeScope freezeScope = this.tickFreezeScope;

        if (freezeScope != null)
        {
            freezeScope.clearScheduledActivity(this.requireSite().regions(), Objects.requireNonNull(this.origin, "origin"));
        }
    }
