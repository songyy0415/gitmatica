...
import me.arnavpmr.lvc.overlay.LvcTrackingOverlayRevisionResolver;
import me.arnavpmr.lvc.overlay.LvcTrackingOverlayRevisionTarget;
...
public final class LvcSemanticOverlayTask extends LvcChunkedTaskBase<LvcTrackingOverlay>
{
    ...
    @Nullable private final LvcTrackingOverlayRevision requestedRevision;
    @Nullable private LvcTrackingOverlayRevisionTarget revisionTarget;
    ...
    @Nullable private LvcTrackingOverlay overlay;
    ...
    public LvcSemanticOverlayTask(LvcOperationHandle handle, Path repositoryDirectory, String projectName,
                                  ...
                                  LvcTaskCallbacks<LvcTrackingOverlay> callbacks)
    {
    ...
    public LvcSemanticOverlayTask(LvcOperationHandle handle, Path repositoryDirectory, String projectName,
                                  @Nullable ClientLevel clientLevel, @Nullable ICompletionListener completionListener,
                                  boolean startVerifier, LvcTrackingOverlayRevision requestedRevision,
                                  LvcTaskCallbacks<LvcTrackingOverlay> callbacks)
    {
    ...
    private LvcSemanticOverlayTask(LvcOperationHandle handle, Path repositoryDirectory, String projectName,
                                   @Nullable ClientLevel clientLevel, @Nullable ICompletionListener completionListener,
                                   boolean startVerifier, boolean forceRebuild,
                                   @Nullable LvcTrackingOverlayRevision requestedRevision,
                                   LvcTaskCallbacks<LvcTrackingOverlay> callbacks)
    {
    ...
    public void init()
    {
        try
        {
            ...
            {
                this.revisionTarget = LvcTrackingOverlayRevisionResolver.resolve(
            ...
            this.overlayName = this.revisionTarget != null ?
                    LvcTrackingOverlayRevisionResolver.displayName(this.manifest.name(), this.revisionTarget) :
            ...
            me.arnavpmr.lvc.LvcDiagnostics.debug(this.handle(), "semantic overlay initialized source={} commit='{}' site={} chunks={} forceRebuild={}",
    ...
    protected boolean step() throws Exception
    {
        ...
        if (this.phase == Phase.LOAD_CACHE)
        {
            ...
            me.arnavpmr.lvc.LvcDiagnostics.debug(this.handle(), "semantic overlay cache loaded repo='{}' elapsedMs={}",
        ...
        if (this.phase == Phase.WRITE_CACHE)
        {
            ...
            this.yieldAfterStep = true;
            me.arnavpmr.lvc.LvcDiagnostics.debug(this.handle(), "semantic overlay cache written repo='{}' file='{}' elapsedMs={}",
        ...
        if (this.phase == Phase.RELOAD_CACHE)
        {
            ...
            this.phase = Phase.ADD_OVERLAY;
            this.yieldAfterStep = true;
            me.arnavpmr.lvc.LvcDiagnostics.debug(this.handle(), "semantic overlay cache reloaded repo='{}' file='{}' elapsedMs={}",
        ...
        if (this.phase == Phase.ADD_OVERLAY)
        {
            ...
            if (this.requestedRevision != null)
            {
                LvcTrackingOverlayRevisionTarget target = this.requireRevisionTarget();
            ...
            me.arnavpmr.lvc.LvcDiagnostics.debug(this.handle(), "semantic overlay placement added repo='{}' startVerifier={} elapsedMs={}",
    ...
    @Override
    protected LvcTrackingOverlay result()
    {
    ...
    protected void updateProgressHud()
    {
        ...
        {
            this.infoHudLines.add(StringUtils.translate("gitmatica.gui.label.lvc_project.task_chunks", this.buildSession.processedChunks(), this.buildSession.totalChunks()));
        }
    }
    ...
    {
        return LvcGitBranchOps.hasUncommittedChanges(this.repositoryDirectory) &&
    ...
    private LvcManifest readCommittedManifest(String commitId) throws Exception
    {
        ...
        this.sourceCommit = LvcGitTreeReader.resolveCommit(repository, this.revWalk, commitId);
        return LvcSemanticRepository.readCommitManifest(repository, this.sourceCommit);
    }

    private LvcTrackingOverlayRevisionTarget requireRevisionTarget()
    {
    ...
    private String overlaySourceDescription()
    {
        if (this.revisionTarget != null)
        {
            return this.revisionTarget.airSchematic() ? "parent-air" :
                    this.requestedRevision == LvcTrackingOverlayRevision.CURRENT ? "current-commit" : "parent-commit";
        }
    ...
    private byte[] readSourceObject(String objectId) throws java.io.IOException
    {
        ...
        byte[] bytes = LvcGitTreeReader.readCommitFile(this.requireRepository(), this.requireSourceCommit(),
